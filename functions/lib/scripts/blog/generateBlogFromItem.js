...
const logger_1 = require("../../lib/logger"); // ✅ 差し替え
...
const generateBlogFromItem = async (item) => {
    ...
    logger_1.logger.info("🟡 投稿処理開始", { sourceItemCode: item.sourceItemCode });
    if (!item.productName || !item.sourceItemCode) {
        logger_1.logger.error("❌ 必須フィールドが不足しています", { item });
        return;
    }
    ...
    try {
        ...
        logger_1.logger.info("✅ テンプレート読み込み成功", { category });
    }
    catch (err) {
        logger_1.logger.error("❌ テンプレート読み込み失敗", { category, error: err });
        return;
    }
    ...
    if (!prompt) {
        logger_1.logger.warn("⚠️ 空のプロンプトが生成されました。処理スキップ", { item });
        return;
    }
    logger_1.logger.debug("🧠 ChatGPT 入力プロンプト", prompt);
    let content = "";
    try {
        ...
        logger_1.logger.error("❌ ChatGPT APIエラー", err);
        return;
    }
    if (!content) {
        logger_1.logger.warn("❌ ChatGPTの出力が空です", { item });
        return;
    }
    logger_1.logger.success("📝 ChatGPT出力あり、投稿処理へ進みます");
    const now = new Date();
    ...
    try {
        ...
        logger_1.logger.success(`✅ ブログ生成完了 → /blogs/${slug}`);
    }
    catch (err) {
        logger_1.logger.error("❌ Firestoreへの保存失敗", err);
    }
};
